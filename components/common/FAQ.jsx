...
const FAQ = () => {
  ...
  const [isOpen, setIsOpen] = useState(Array(faqItems.length).fill(false));

  //   --Function to toggle the answers which are hidden--
  const toggleFAQItem = (index) => {
    const updateData = [...isOpen];
    updateData[index] = !updateData[index];
    setIsOpen(updateData);
  };

  return (
    <div className='h-full w-full bg-blue-950 py-8 sm:py-12 px-5 sm:px-16'>
      {/* --Title-- */}
      <div>
        <h1
          ...
          <div
            ...
            <div className='flex items-center justify-between mb-2'>
              ...
              <button onClick={() => toggleFAQItem(index)}>
                {isOpen[index] ? <FaMinus /> : <GrAdd />}
              </button>
            </div>
            {isOpen[index] && <p className='text-gray-600'>{item.answer}</p>}
          </div>
        ))}
      </div>
    </div>
